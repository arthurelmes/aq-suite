...
def measure_co2_temp() -> Dict:
    ...
    co2_temp = {
        "co2": raw_reading.get("co2", FILL_VALUE),
        "temp_c": raw_reading.get("temperature", FILL_VALUE),
    }
    return co2_temp
...
if __name__ == "__main__":
    ...
    args["end_datetime"] = args["start_datetime"] + timedelta(
        hours=args["logging_time_hours"]
    )

    pretty_args = pprint.pformat(args)
    logger.info("Running with the following configuration:\n%s", pretty_args)
    add_log_file_header(
        log_file_path, args["notes"], cols=data_cols, config=pretty_args
    )

    logger.info("measurement_time\tco2\ttemp_c\tpm2.5\tpm10")
